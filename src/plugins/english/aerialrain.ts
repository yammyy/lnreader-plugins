...
class aerialrainPlugin implements Plugin.PluginBase {
  ...
  async popularNovels(pageNo: number): Promise<Plugin.NovelItem[]> {
    if (pageNo > 1) return []; // Only fetch the first page
  ...
  async parseNovel(novelPath: string): Promise<Plugin.SourceNovel> {
    ...
    // =========================
    // 2) ORIGINAL TITLE, AUTHOR, GENRES
    // =========================
    ...
    // 2) Remove all <br> tags
    firstP.find('br').remove();

    // 3) Get bare text (strip all remaining tags)
    let text = firstP.text().trim();
    ...
    // 5) Extract Original Title, Author, Genre using simple regex
    let originalTitle = '';
    let author = '';
    let rawGenres = '';
